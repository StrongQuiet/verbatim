        ...
        {status === "initial" ? (
          <div className="content-inner flex">
            <h2 className="content__title">
              {state.title} <span>({state.lvl})</span>
            </h2>
            <button className="content__btn" onClick={() => started()}>
              Начать
            </button>
          </div>
        ) : status === "started" ? (
          <div className="content-inner flex">
            ...
            {statusHelp === 1 ? (
              <div className="content__btn help flex">
                ...
                <div onClick={() => setStatusHelp(() => 2)}>
                  <span>Ответ</span>
                </div>
              </div>
            ) : statusHelp === 2 ? (
              <div className="content__btn help">
                <h2>{words[wordId].translate}</h2>
              </div>
            ) : (
              <div
                className="content__btn help"
                onClick={() => setStatusHelp(() => 1)}
              >
                <span>Подсказка</span>
              </div>
            )}
            <button className="content__btn" onClick={() => check()}>
              Проверить
            </button>
          </div>
        ) : (
